...
export default function Products() {
  ...
  const content = useMemo(() => {
    switch (foodListLoadable.state) {
      ...
      case "hasValue": {
        ...
        if (total === 0) {
          return <Alert severity="info">По заданным фильтрам товаров не найдено</Alert>;
        }
        return (
          <>
            <Grid2 container spacing={4}>
              {items.map((food: FoodProps, idx: number) => (
                <ProductItem key={food.id} item={food} i={idx} />
              ))}
            </Grid2>
            {total > PAGE_SIZE && (
  ...
  return (
    <Grid2 container spacing={3} p={3}>
      ...
      {/* Sort & Desktop Filters */}
      <Grid2 size={{ xs: 6, sm: 3, md: 3 }}>
        {totalItems > 0 && <ProductSort />}
      </Grid2>
      <Grid2 size={{ md: 3 }} sx={{ display: { md: "block", sm: "none", xs: "none" } }}>
        <ProductFilter />
      </Grid2>
