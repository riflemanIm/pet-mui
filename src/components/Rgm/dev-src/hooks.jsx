...
const GOOGLE_API_KEY = 'AIzaSyAGKL9rneCeNbkkiYAxdcfdOapefZdVZRU';
...
export const getMapApiUrl = (appInfoMapsKey) => {
  const key = appInfoMapsKey || GOOGLE_API_KEY;
  //   if (key == null) {
  //     throw new Error("process.env.GOOGLE_API_KEY is not provided");
  //   }
  //console.log('appInfoMapsKey', appInfoMapsKey);
  const url = `https://maps.googleapis.com/maps/api/js?key=${key}&callback=rgm_mapLoaded`;
  return url;
};

export const useGoogleApiLoader = (appInfoMapsKey) => {
  const [api, setApi] = React.useState(map_.api);
  const url = getMapApiUrl(appInfoMapsKey);
