...
const FoodList = () => {
  ...
  const columns = useMemo<GridColDef<FoodDto>[]>(
    () => [
      { field: "id", headerName: "ID", width: 80 },
      {
        ...
        getActions: (params: GridRowParams<FoodDto>) => [
          <GridActionsCellItem
            ...
            icon={<CreateIcon />}
            label="Edit"
            onClick={() => navigate(`/food/${params.id}/edit`)}
          />,
          <GridActionsCellItem
            key="del"
            icon={<DeleteIcon />}
      ...
      },
      {
        ...
        width: 160,
        renderCell: (p) => (
          <span>{nameOrId(refs?.taste, (p.row as any)?.tasteId)}</span>
        ),
      },
      {
        field: "ingredientId",
        ...
        renderCell: (p) => (
          <span>
            {nameOrId(refs?.ingredient, (p.row as any)?.ingredientId)}
          </span>
        ),
      },
      {
        ...
        width: 160,
        renderCell: (p) => (
          <span>{nameOrId(refs?.hardness, (p.row as any)?.hardnessId)}</span>
        ),
      },
      {
        ...
        minWidth: 220,
        renderCell: (p) =>
          chipsByIds(refs?.designedFor, (p.row as any)?.designedForIds),
      },
      {
        field: "ageIds",
        headerName: "Возраст",
      ...
      },
      {
        ...
        minWidth: 180,
        renderCell: (p) =>
          chipsByIds(refs?.typeTreat, (p.row as any)?.typeTreatIds),
      },
      {
        field: "petSizeIds",
        ...
        minWidth: 180,
        renderCell: (p) =>
          chipsByIds(refs?.petSizes, (p.row as any)?.petSizeIds),
      },
      {
        field: "packageIds",
        headerName: "Упаковки",
        ...
        minWidth: 180,
        renderCell: (p) =>
          chipsByIds(refs?.packages, (p.row as any)?.packageIds),
      },
      {
        ...
        minWidth: 220,
        renderCell: (p) =>
          chipsByIds(refs?.specialNeeds, (p.row as any)?.specialNeedsIds),
      },
    ],
    [navigate, refs]
  );
  ...
  const applyFilter = () =>
    actions.doFetch(0, undefined, filter || null)(dispatch);
